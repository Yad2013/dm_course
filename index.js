...
const lngd=document.getElementById("lng");
...
function load_page(f){
  ...
  vid.innerHTML=rv;
  if(stt.innerHTML.length==0) vid.style.height="100vh";
  ...
  fft=ftd==1?`<a onclick="toggle_full_text(1)">Show</a>`:`<a onclick="toggle_full_text(0)">Hide</a>`;
  var more=`<div id=mor><p>Full Text: ${fft}</p><p>Speed: ${spt}</p><p><a onlcick="show_about()">About</a></p></div>`;
  ftr.innerHTML=more+PGB+FTB;
  lngd.innerHTML="";
  if(i==0) document.getElementById("vpb").addEventListener("click", play_slides);
  // stt.addEventListener("dblclick", edit_subtitles);
  if(f==0 && i==0) lngd.innerHTML=FLA[lng];
  let ia=i;
  if(lng!=2) ia=en_to_ar(i);
  if(i>0) lngd.innerHTML=ia;
...
document.addEventListener("keydown",event=>{
  let f=0;
  if(event.target.id=="txt") f=1;
  if(event.target.type=="text") f=1;
  if(event.target.type=="radio") f=1;
  // if(f==0) event.preventDefault();
  // console.log(event.key, event.keyCode, event.target.id, event.target.type);
  ...
  // if(event.key==="ArrowLeft") prev_step();
  if(event.key==="ArrowRight") seek(5);
  if(event.key==="ArrowLeft") seek(-5);
  ...
  if(event.key==="t") toggle_full_text(1);
...
var mns=0;
function vid_dblclick(){
  let s=event.clientX<window.innerWidth/2?-5:5;
  mns=1;
  seek(s);
  }
...
function rbc(id){ // radio button clicked
  sid=id;
  if(!document.getElementById(sid).classList.contains("c")) return false;
...
function logged_in(){
  let rv = appStore.auth.state.loggedIn;
  return rv;
  }
...
function show_related_link(){
  ...
  // hide_menu();
  // if(get_related()!=false && subs==0) document.getElementById("lng").innerHTML=`<a onclick="toggle_related()">${INF}</a>`;
  }

function audio_ended() {
  if(stsa.includes(i)) {
    if(i==7 && subs==1) return true;
    document.getElementById("cb").style="opacity:1;";
    document.getElementById("cb").disabled=false;
    return true;
    }
...
function get_more(){
  ...
  document.getElementById("mor").style=`bottom:66px;height:146px;left:${lm}px;border:1px #ccc solid;border-bottom:0;`
  clearTimeout(toh);
  }
...
function show_menu(f){
  ...
  if(f) toh=setTimeout(()=>{hide_menu();}, 4000);
  }
  
function lng_click(){
  if(i>0) toggle_full_text(1);
}
